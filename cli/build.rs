...
fn main() {
  ...
  println!(
    "cargo:rustc-link-arg-bin=subshell=/DEF:{}",
    symbols_path.display()
  );

  #[cfg(target_os = "macos")]
  println!(
    "cargo:rustc-link-arg-bin=subshell=-Wl,-exported_symbols_list,{}",
    symbols_path.display()
  );
  ...
  #[cfg(target_os = "linux")]
  {
        ...
        .unwrap_or(false)
    {
      ...
      println!("cargo:rustc-link-arg-bin=subshell=-rdynamic");
    } else {
      println!(
        "cargo:rustc-link-arg-bin=subshell=-Wl,--export-dynamic-symbol-list={}",
        symbols_path.display()
      );
    }
  }
